        ...
        module.exports={
            postRegister: async (req, res) => {
              ...
              if (!name||!email || !password) {
                return res.status(401).send({ error: "Provide all values" ,success:false  });
              }
              try {
                ...
                if (user) {
                  ...
                  res.status(401).json({error:"Email Already Exists",success:false})
                }
          else{
          ...
          if (!user) {
            return res.status(404).send({ error: "User Dont Exists" ,success:false });
          }
